...
function openCvReady() {
    cv['onRuntimeInitialized'] = () => {
        ...
        let modelURL = 'https://teachablemachine.withgoogle.com/models/5s1hAx9Ls/';
        ...
        for (let i = 0; i < faces.size(); i++) {
            ...
            let xr = this.tamanno.width / sizeFacemat.width
            let yr = this.tamanno.height / sizeFacemat.height
            // eslint-disable-next-line no-undef
            let point1 = new cv.Point(face.x * xr, face.y * yr)
            // eslint-disable-next-line no-undef
            let point2 = new cv.Point((face.x + face.width) * xr, (face.y + face.height) * yr)
            // eslint-disable-next-line no-undef
            cv.rectangle(this.canvas, point1, point2, [255, 0, 0, 255])
            if (faces.size() === 1) {
              if (window.innerHeight > 600) {
                if (face.height > 45) {
                  // eslint-disable-next-line no-undef
                  let ts1 = window.performance && window.performance.now && window.performance && window.performance.timing.navigationStart ? window.performance.now() + window.performance.timing.navigationStart : Date.now()
                  this.tomarFoto(ts1)
                }
              } else if (face.height > 55) {
                // eslint-disable-next-line no-undef
                let ts1 = window.performance && window.performance.now && window.performance && window.performance.timing.navigationStart ? window.performance.now() + window.performance.timing.navigationStart : Date.now()
                this.tomarFoto(ts1)
              }
        ...
        //Loading the model with async as loading the model may take few milliseconds
